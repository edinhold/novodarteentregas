  ...
  deliveryRequests.forEach((req) => {
    ...
    if (inPeriod) {
      driverEarningsList.push({
        ...
        driverName,
        driverPhone,
        storeId: resolvedRestId || req.restaurant_id || "",
        storeName,
        rideTime,
      ...
      transactions.push({
        ...
        description: `Entrega #${req.id.slice(0, 8)} (${storeName})`,
        partyName: `${driverName} (Loja: ${storeName})`,
        partyRole: "system",
        storeName,
        driverName,
  ...
  withdrawalRequests.forEach((w) => {
    ...
    if (inPeriod) {
      transactions.push({
        ...
        date: dateStr,
        type: "saque",
        typeLabel: w.fee_amount > 0 ? "Antecipação" : "Saque PIX",
