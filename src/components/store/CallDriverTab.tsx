...
const CallDriverTab = ({ user, restaurant, requests, activeRequest, chatMessages, credits, onNavigateTab }: CallDriverTabProps) => {
  ...
  // Query to resolve driver names and plates for history items
  ...
  const getDriverInfo = (driverId: string | null | undefined) => {
          ...
          {visibleRequests.length === 0 ? (
            ...
            <div className="space-y-2">
              {visibleRequests.map((r: any) => {
                const driver = getDriverInfo(r.driver_id);
                const driverName = driver?.full_name || r.driver?.full_name || (r.driver_id ? "Motorista Atribuído" : null);
                  ...
                  <div key={r.id} className="p-3.5 sm:p-4 rounded-xl bg-card hover:bg-muted/40 transition-colors border border-border/70 shadow-sm space-y-3">
                    ...
                    <div className="flex justify-between items-start gap-2 flex-wrap pb-2 border-b border-border/40">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-extrabold font-mono text-primary">#{r.id.slice(0, 8)}</span>
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        {deliveryVal > 0 && (
                          <span className="text-xs font-black text-emerald-700 dark:text-emerald-300 bg-emerald-500/10 px-2.5 py-1 rounded-md border border-emerald-500/30 flex items-center gap-1 shadow-xs">
                            <DollarSign className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                            <span>R$ {deliveryVal.toFixed(2).replace(".", ",")}</span>
                          </span>
                        )}

                        <Badge variant={r.status === "delivered" ? "default" : r.status === "cancelled" ? "destructive" : "secondary"} className="font-semibold px-2.5 py-0.5">
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1 flex-wrap">
                      ...
                      {/* Botão de Limpar (Remover do Histórico) */}
                      <Button
                        size="sm"
                        variant="outline"
                        className="h-8 px-2.5 text-xs text-muted-foreground hover:text-destructive hover:bg-destructive/10 border-border/60 gap-1.5 ml-auto"
                        onClick={() => handleDeleteSingleRequest(r.id)}
                        disabled={deletingRequestId === r.id}
                        title="Limpar esta entrega do histórico da sua loja"
                      >
