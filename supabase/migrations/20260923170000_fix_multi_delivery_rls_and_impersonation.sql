-- Migration: Fix Multi-Delivery Group RLS policies and Impersonation support in RPC
-- 1. DROP old SELECT policies on delivery_groups if any
DROP POLICY IF EXISTS "Store owners can view their own groups" ON public.delivery_groups;
DROP POLICY IF EXISTS "Drivers can view their assigned groups" ON public.delivery_groups;
DROP POLICY IF EXISTS "Drivers can view pending groups and assigned groups" ON public.delivery_groups;

-- 2. CREATE updated SELECT policy allowing drivers to view pending groups where driver_id IS NULL
CREATE POLICY "Drivers and Store Owners can view related and pending groups"
  ON public.delivery_groups FOR SELECT TO authenticated
  USING (
    store_owner_id = auth.uid()
    OR driver_id = auth.uid()
    OR (status = 'pending' AND (driver_id IS NULL OR driver_id = auth.uid()))
    OR public.has_role(auth.uid(), 'admin'::app_role)
  );

-- 3. UPDATE RPC FUNCTION: create_delivery_group to support impersonation and robust pricing
...
BEGIN
  ...
  -- Resolve real store owner ID from restaurant, with fallback to caller
  SELECT owner_id INTO v_store_owner_id FROM public.restaurants WHERE id = p_restaurant_id;
  IF v_store_owner_id IS NULL THEN
    v_store_owner_id := v_caller_id;
  END IF;

  -- Validate caller is either the store owner or an admin
  IF v_caller_id <> v_store_owner_id AND NOT public.has_role(v_caller_id, 'admin'::app_role) THEN
    RAISE EXCEPTION 'Sem permissão para criar entregas para esta loja';
  END IF;
  ...
  SELECT balance INTO v_balance FROM public.store_credits WHERE user_id = v_store_owner_id;
  ...
  UPDATE public.store_credits SET balance = balance - v_total_cost, updated_at = now() WHERE user_id = v_store_owner_id;
  ...
  INSERT INTO public.delivery_groups (
    ...
    v_store_owner_id, p_restaurant_id, p_pickup_address, p_preferred_driver_id, v_total_cost, v_stops_count, p_group_notes
  ...
  FOR v_stop IN SELECT * FROM jsonb_array_elements(p_stops) LOOP
    ...
    INSERT INTO public.delivery_requests (
      ...
      v_store_owner_id, p_restaurant_id, p_pickup_address, v_stop->>'delivery_address', v_stop->>'notes',
...
GRANT EXECUTE ON FUNCTION public.create_delivery_group(uuid, text, jsonb, uuid, text) TO authenticated, service_role, anon;
