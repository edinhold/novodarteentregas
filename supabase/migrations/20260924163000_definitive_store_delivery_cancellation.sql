-- Migration: Definitive Store Delivery Request and Multi-Delivery Group Cancellation Flow
-- Fixes RPC overload resolution, permissions checks, status checks, credit refunds, and driver earnings cleanup.
...
BEGIN
  ...
  IF v_group.status = 'delivered' THEN
    RAISE EXCEPTION 'Esta entrega já foi concluída e não pode ser cancelada.';
  END IF;

  -- Flexible permission check: match store_owner_id, restaurant owner, store role, assigned driver, admin or any authenticated store owner
  v_is_store_owner := (
...
BEGIN
  ...
  -- Permission check: match store_owner_id, restaurant owner, store role, assigned driver, admin or any authenticated user
  v_is_store_owner := (
