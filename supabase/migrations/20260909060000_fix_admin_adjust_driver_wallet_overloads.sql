-- Migration: Fix admin_adjust_driver_wallet overloads & PostgREST schema cache reload

-- 1. Main function signature (uuid driver_id)
...
BEGIN
  ...
  -- 5. Calculate signed amount and new balance
...
-- 2. Overload signature (p_driver_id text)
CREATE OR REPLACE FUNCTION public.admin_adjust_driver_wallet(
  p_driver_id text,
  p_operation text,
  p_amount numeric,
...
AS $$
BEGIN
  RETURN public.admin_adjust_driver_wallet(
    p_driver_id::uuid,
    p_operation,
    p_amount,
    p_reason,
...
-- 3. Overload signature (p_amount numeric first, p_driver_id uuid)
CREATE OR REPLACE FUNCTION public.admin_adjust_driver_wallet(
  p_amount numeric,
  p_driver_id uuid,
...
-- 4. Overload signature (p_amount numeric first, p_driver_id text)
CREATE OR REPLACE FUNCTION public.admin_adjust_driver_wallet(
  p_amount numeric,
  p_driver_id text,
  p_idempotency_key text,
...
-- 5. Grant permissions to authenticated, anon, service_role on ALL overloads
GRANT EXECUTE ON FUNCTION public.admin_adjust_driver_wallet(uuid, text, numeric, text, text) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.admin_adjust_driver_wallet(text, text, numeric, text, text) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.admin_adjust_driver_wallet(numeric, uuid, text, text, text) TO authenticated, anon, service_role;
GRANT EXECUTE ON FUNCTION public.admin_adjust_driver_wallet(numeric, text, text, text, text) TO authenticated, anon, service_role;
