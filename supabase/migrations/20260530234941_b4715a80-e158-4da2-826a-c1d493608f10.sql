-- Drop outdated overloads to ensure we are using the most recent version
DROP FUNCTION IF EXISTS public.deduct_credits_for_delivery(text, text, text, uuid);
DROP FUNCTION IF EXISTS public.deduct_credits_for_delivery(text, text, text, uuid, numeric);

-- Recreate the function with the correct column name 'balance'
...
BEGIN
  ...
  -- Check credits (Corrected column name from amount to balance)
  ...
  -- Deduct credits (Corrected column name from amount to balance)
  ...
  IF p_preferred_driver_id IS NOT NULL THEN
    -- Check if there's a driver_id column or similar. For now we use the logic from previous version.
